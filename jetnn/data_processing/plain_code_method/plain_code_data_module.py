...
from jetnn.data_processing.plain_code_method.plain_code_dataset import PlainCodeDataset


class PlainCodeDataModule(LightningDataModule):
    _train = "train"
    _val = "val"
    _test = "test"
    ...
    def _create_dataset(self, holdout_file: str) -> PlainCodeDataset:
        if self._vocabulary is None:
            raise RuntimeError(f"Setup vocabulary before creating data loaders")
        return PlainCodeDataset(holdout_file, self._config, self._vocabulary)
