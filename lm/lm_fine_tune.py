...
def load_bart(dataset):
    ...
    return model, tokenizer, dataset_train, dataset_val, dataset_test


def compute_loss(model, dataset, tokenizer, use_grad=False):
    with torch.set_grad_enabled(use_grad):
        model_logits = model(input_ids=dataset[0]['input_ids'], attention_mask=dataset[0]['attention_mask'], decoder_input_ids=dataset[1]['input_ids'], decoder_attention_mask=dataset[1]['attention_mask']).logits)
        cross_entropy_tensor = functional.cross_entropy(model_logits[:,:-1].reshape(-1, model_logits.shape[-1]), dataset[1]['input_ids'][:,1:].reshape(-1),reduction="none", ignore_index=tokenizer.pad_token_id).reshape(model_logits.shape[0],model_logits.shape[1]-1)



if __name__ == "__main__":
    metric = evaluate.load("accuracy")
    model, tokenizer, dataset_train, dataset_val, dataset_test = load_bart("tau/scrolls", 'quality')
